...
function pckgManager.header() -- header
	pckgManager.print(
		pckgManager.printLevel.message,
		"===================================================================================="
	) -- separator
...
function pckgManager.setup() -- setup function ( setup whole project automatically ) with creating sources file
	...
	table.insert(
		defaultSources,
		"https://raw.githubusercontent.com/Bluescreen-yt/CardBoard-OS-CCT/refs/heads/main/core.json"
	) -- cbos
...
function pckgManager.init_config() -- initial default config
	...
	pckgManager.installPaths =
		{ -- this table contains paths to `different` types of packages like applications or services
...
function pckgManager.install(package, version, installing) -- install package
	-- bs to do: rewrite whole funtion
	...
	installing[package] = version or true

	pckgManager.print(
		pckgManager.printLevel.message,
		"pckgManager.install( '" .. (package or "nil") .. "', '" .. (version or "nil") .. "' ) - start"
	)
	...
	if not pckg_data then
		pckgManager.print(
			pckgManager.printLevel.error,
			"could not get " .. version .. " version of the package " .. package,
			debug.getinfo(1, "Sl")
		)
	...
	if not pckgFileData then
		pckgManager.print(
			pckgManager.printLevel.error,
			"failed to retrive package data from url: " .. pckg_data,
			debug.getinfo(1, "Sl")
		)
	...
	if not fileLocation then
		pckgManager.print(
			pckgManager.printLevel.error,
			"invalid package type: " .. pckgFileData.pckg_type,
			debug.getinfo(1, "Sl")
		)
	...
	for file, url in pairs(pckgFileData.files) do
		...
		table.insert(funcs, function() -- add func to funcs table
			...
			if not fileRequest then
				pckgManager.print(
					pckgManager.printLevel.error,
					"failed to retrive file: " .. file .. " from url: " .. url,
					debug.getinfo(1, "Sl")
				)
	...
	for pckg, version in pairs(pckgFileData.requirements) do -- install all requirements
		pckgManager.install(pckg, version, installing)
	end
	...
	if pckgFileData.autorun_file then
		...
		local success, result = pcall(function()
			...
			if not autorunCodeAsFunc then
				pckgManager.print(
					pckgManager.printLevel.error,
					"failed to load autorun file: " .. autorunPath,
					debug.getinfo(1, "Sl")
				)
				return false, false
			end
		...
		if not (success and result) then
			pckgManager.print(
				pckgManager.printLevel.error,
				"file no found, or error with requiring it ( autorunCodeAsFunc is nil / false )",
				debug.getinfo(1, "Sl")
			)
		...
		if type(result) == "table" and result[pckgFileData.autorun_function] then
			...
			if not success then
				pckgManager.print(
					pckgManager.printLevel.error,
					"failed to run autorun function",
					debug.getinfo(1, "Sl")
				)
	...
	for package, aliases in pairs(pckgManager.aliases) do
		for alias, file in pairs(aliases) do
			if shell then
				...
				pckgManager.print(
					pckgManager.printLevel.warning,
					"shell not found, are you running with custom globals / env?"
				)
			end
		end
	end
...
function pckgManager.PMNG.loadPlugins(pathOverride)
	...
	if not fs.exists(path) then
		pckgManager.print(pckgManager.printLevel.warning, "no plugins path found")
		return
	end

	local plugins = fs.list(path)
	if #plugins == 0 then
		pckgManager.print(pckgManager.printLevel.message, "no plugins found")
		return
	end
...
function GH.init()
	AssignUrlOverite({
		...
		PackageSource = function(url)
			...
			local user, repo = string.match(url, "github.com/([^/]+)/([^/]+)")
			local requestUrl = GH.API_URL .. "/repos/" .. user .. "/" .. repo .. "/releases"
			local GHRequest = http.get(requestUrl)
			local Versions = GHRequest.readAll()
			GHRequest.close()

			Versions = textutils.unserialiseJSON(Versions)
